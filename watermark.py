...
class watermark:
    """ A watermark class customized with the reportlab module.
     Attributes:
        ...
        watermark_it
    """
    _papertype = {"A4":[210,297], "A0": [841, 1189], "A1": [594, 841], "A2": [420, 594], "A3": [297, 420], "A5": [148,210]} # mm
    ...
    def getPageExtend(self, page = 0):
        self.pagesize = [int(round(float(row)*self._papertype_conversion_factor)) for row in pyPdf.PdfFileReader(file(self.fname,'rb')).getPage(page).mediaBox[2:4]]
        self.getPageSize()


    def getPageSize(self):
        for row in self._papertype.keys():
            if (self._papertype[row][0]-2 < self.pagesize[0] < self._papertype[row][0]+2) and  (self._papertype[row][1]-2 < self.pagesize[1] < self._papertype[row][1]+2):
                self.pagetype = row
                print row
            elif (self._papertype[row][1]-2 < self.pagesize[0] < self._papertype[row][1]+2) and  (self._papertype[row][0]-2 < self.pagesize[1] < self._papertype[row][0]+2):
